...
class Cortex {
  constructor({ model, client, memory, selfModel, tools, worldModel, procedures, maxSteps = DEFAULT_MAX_STEPS }) {
    this.model = model;
    this.client = client;
    this.memory = memory;
  ...
  toolManifest() {
    return this.tools.list().map(t => ({ name: t.name, description: t.description, parameters: t.parameters || {}, risk: t.risk || "unknown" }));
  }
...
{"thought_summary":"brief reasoning","decision":"respond|act|ask|learn|finish","message":"user-facing response","tool_calls":[{"name":"tool","arguments":{}}],"memory_candidates":[{"kind":"fact|episode|procedure|preference","content":"..."}],"self_updates":[{"field":"...","value":"..."}],"confidence":0.0}`;
  }
  ...
  async decide({ session, userText, observations = [] }) {
    ...
    const completion = await this.client.chat.completions.create({
      ...
      messages: [{ role: "system", content: this.systemPrompt() }, { role: "user", content: safeJson(context) }]
    });
  ...
  async run({ session, userText, approvalToken = null }) {
    ...
    for (let step = 0; step < this.maxSteps; step++) {
      ...
      if (decision.decision !== "act" || !decision.tool_calls.length) {
        ...
        return { ...decision, decisions, observations, learning: await this.learning?.learnFromOutcome(session.id, { goal: userText, plan: decisions.map(d => d.tool_calls || []), observations, outcome, procedureId }) };
      }
    ...
    return { decision: "respond", message: "I could not safely complete the task within the current execution cycle. I preserved the observations for future reasoning.", tool_calls: [], decisions, observations };
  }
}
