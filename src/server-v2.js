...
const { Executive } = require("./cognitive/executive");
const { LearningEngine } = require("./cognitive/learning");
...
const { ProcedureStore } = require("./cognitive/procedure-store");
const { CapabilityBus } = require("./cognitive/capability-bus");
const { WorldModel } = require("./cognitive/world-model");
const { Authority } = require("./cognitive/authority");
...
const procedures = new ProcedureStore(supabase);
const capabilityBus = new CapabilityBus(supabase);
const worldModel = new WorldModel(supabase);
...
const executive = new Executive({ cortex, learning, selfModel });
...
app.post("/api/capabilities", async (req, res) => {
  ...
  try {
    ...
    for (const capability of incoming) {
      ...
      tools.register({
        name: saved.name, description: saved.description, parameters: saved.parameters, risk: saved.risk,
        execute: async args => ({ dispatch: true, name: saved.name, arguments: args }),
        authorize: async (args, context) => (await authority.authorize(saved, args, context)).allowed
      });
    }
    await worldModel.registerCapabilities(sessionId, registered);
    res.json({ ok: true, capabilities: await capabilityBus.list(sessionId) });
...
app.post("/api/world", async (req, res) => {
  const sessionId = String(req.body?.sessionId || "").trim();
  if (!sessionId) return res.status(400).json({ error: "sessionId required" });
  try { res.json({ ok: true, world: await worldModel.observe(sessionId, req.body?.observation || {}) }); }
  catch (error) { res.status(500).json({ ok: false, error: error.message }); }
});

app.get("/api/health", async (_req, res) => {
  const { error } = await supabase.from("jarvis_sessions").select("session_id").limit(1);
  res.json({ ok: true, brain: "cognitive-v1", database: error ? "unreachable" : "connected", capabilities: tools.list().length });
});

app.get("/api/self", async (req, res) => {
  const sessionId = String(req.query.sessionId || "");
  if (!sessionId) return res.status(400).json({ error: "sessionId required" });
  try { res.json(await selfModel.ensure(sessionId)); } catch (error) { res.status(500).json({ error: error.message }); }
});

app.get("/api/world", async (req, res) => {
  const sessionId = String(req.query.sessionId || "");
  if (!sessionId) return res.status(400).json({ error: "sessionId required" });
  try { res.json(await worldModel.snapshot(sessionId)); } catch (error) { res.status(500).json({ error: error.message }); }
});

app.get("/api/memory", async (req, res) => {
  const sessionId = String(req.query.sessionId || "");
  if (!sessionId) return res.status(400).json({ error: "sessionId required" });
  try { res.json(await memory.recall(sessionId, String(req.query.q || ""), 50)); } catch (error) { res.status(500).json({ error: error.message }); }
});

app.post("/api/chat", async (req, res) => {
  const sessionId = String(req.body?.sessionId || "").trim();
  const message = String(req.body?.message || "").trim();
  const approvalToken = req.body?.approvalToken || null;
