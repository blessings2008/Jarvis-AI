...
function buildSystemPrompt(session, extra = {}) {
    ...
    const searchBlock = extra.searchResults
        ...
        : extra.searchFailed
        ? `Your web search for "${extra.searchQuery}" just FAILED (the search service returned nothing — could be a config or connectivity issue on the server side). Do NOT say you searched or imply you have results. Tell Bless plainly that the search didn't go through right now, and answer from your own knowledge if you can, or suggest trying again shortly. Set "web_search" to null.`
        : "";
...
// ------------------------------------------------------------
app.get("/", (req, res) => {
    res.json({ status: "online", name: "JARVIS", version: "0.8.1", model: GROQ_MODEL, uptimeSeconds: Math.floor((Date.now() - SERVER_START) / 1000), message: "JARVIS AI backend is running." });
});
...
app.post("/api/chat", async (req, res) => {
    ...
    try {
        ...
        // re-ask with results (success) or a failure note (search came
        // back empty) — either way the reply reflects what actually
        // happened, never the pre-search "I'm about to search" guess.
        if (searchEnabled && parsed && parsed.web_search && typeof parsed.web_search.query === "string" && parsed.web_search.query.trim()) {
            ...
            const searchSystemPrompt = results
                ? buildSystemPrompt(session, { reliability, searchResults: results, searchQuery: query, suppressSearch: true })
                : buildSystemPrompt(session, { reliability, searchFailed: true, searchQuery: query, suppressSearch: true });
            ...
            if (followUpText) {
                const followUpParsed = safeParseJSON(followUpText);
                if (followUpParsed) {
                    parsed = followUpParsed;
                    if (results) sources = results.map((r) => ({ title: r.title, url: r.url }));
                }
            }
            // If the follow-up call itself failed to return usable JSON,
            // `parsed` stays as the original pre-search guess — rare, but
            // better than a hard error on an otherwise-successful turn.
        }
...
const EVOLUTION_LOG = [
    ...
    { version: "0.8.1", capabilities: ["chat", "groq", "actions", "android_shizuku_ready", "memory", "confirmation_flow", "action_verification", "modes", "styles", "skills", "automations", "proactive_nudges", "sensitive_tier", "vision", "supabase_persistence", "permission_status", "capability_gap_logging", "self_authored_suggestions", "action_reliability_tracking", "pattern_based_nudges", "web_search", "honest_search_failures", "crash_resilience"] }
];
...
// ------------------------------------------------------------
// Process-level safety nets — an unhandled promise rejection
// anywhere (e.g. a fire-and-forget DB write) crashes the whole
// Node process by default on modern Node, which is what a
// Render 502 with no JSON body usually means (the process died,
// not that our code returned an error). Log and stay alive
// instead of taking the whole server down over one bad write.
// ------------------------------------------------------------
